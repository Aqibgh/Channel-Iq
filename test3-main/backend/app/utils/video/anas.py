...
AI_models_list = (
            SRVGGNetCompact_models_list + AI_LIST_SEPARATOR + BSRGAN_models_list + AI_LIST_SEPARATOR + IRCNN_models_list)
    ...
    for frame_number in range(frame_count):
        ...
        if success:
            ...
            if len(extracted_frames) == frames_number_to_save:
                ...
                write_process_status(processing_queue,
                                     f"{file_number}. Extracting video frames ({round(percentage_extraction, 2)}%)")
...
def interpolate_images_and_save(
    ...
    try:
        ...
        interpolated_image = opencv_addWeighted(starting_image, starting_image_importance, upscaled_image,
                                                upscaled_image_importance, ZERO)
...
def save_multiple_upscaled_frame_async(
    ...
    for frame_index, _ in enumerate(upscaled_frames_to_save):
        ...
        if selected_interpolation_factor > 0:
            interpolate_images_and_save(upscaled_frame_path, starting_frame, upscaled_frame,
...
def upscale_image(
    ...
    upscaled_image_path = prepare_output_image_filename(image_path, selected_output_path, selected_AI_model,
                                                        resize_factor, selected_image_extension,
                                                        selected_interpolation_factor)
    ...
    if selected_interpolation_factor > 0:
        interpolate_images_and_save(upscaled_image_path, starting_image, upscaled_image, selected_interpolation_factor,
                                    selected_image_extension)
    else:
...
def upscale_video(
    ...
    if video_upscale_continue:
        ...
        write_process_status(processing_queue,
                             f"{file_number}. Resume video upscaling ({len(extracted_frames_paths)} frames)")
    else:
        write_process_status(processing_queue, f"{file_number}. Extracting video frames")
        extracted_frames_paths = extract_video_frames(processing_queue, file_number, target_directory, video_path,
                                                      cpu_number)
    ...
    check_forgotten_video_frames(processing_queue, file_number, AI_instance, extracted_frames_paths,
                                 upscaled_frame_paths, selected_interpolation_factor)
    ...
    for frame_index in range(len((extracted_frames_paths))):
        ...
        if already_upscaled == False:
            ...
            if (frame_index + 1) % 8 == 0:
                ...
                update_process_status_videos(processing_queue, file_number, frame_index, len(extracted_frames_paths),
...
def upscale_video_frames_multithreading(
        ...
        for frame_index in range(len(extracted_frames_paths)):
            ...
            if already_upscaled == False:
                ...
                if (processed_frames_index_async + 1) % (8 * multiframes_number) == 0:
                    ...
                    update_process_status_videos(processing_queue, file_number, processed_frames_index_async,
                                                 total_video_frames, average_processing_time)
    ...
    frame_list_chunks = [extracted_frames_paths[i:i + chunk_size] for i in
                         range(0, len(extracted_frames_paths), chunk_size)]
    upscaled_frame_list_chunks = [upscaled_frame_paths[i:i + chunk_size] for i in
                                  range(0, len(upscaled_frame_paths), chunk_size)]
...
def get_upscale_factor() -> int:
    global selected_AI_model
    if AI_LIST_SEPARATOR[0] in selected_AI_model:
        upscale_factor = 0
...
# Standard imports and functionality will remain unchanged.
import json
import argparse
import sys
import os
def find_latest_file(directory):
    files = [os.path.join(directory, f) for f in os.listdir(directory) if f.endswith(('.png', '.jpg', '.mp4'))]
    if not files:
        return None
    return max(files, key=os.path.getctime) 
def process_media(file_path, ai_model="RealESR_Gx4", resize_factor=50, output_path=None, cpu_number=4, keep_frames=False):
    """
    Process a video or image file using the specified AI model and parameters.

    Args:
        file_path (str): Path to the video or image file to be processed.
        ai_model (str): Name of the AI model to use for processing (default: "RealESR_Gx4").
        resize_factor (int): Resize factor percentage (default: 50).
        output_path (str): Output path for the processed file (default: media directory).
        cpu_number (int): Number of CPU cores to use for processing (default: 4).
        keep_frames (bool): Whether to keep individual frames (only applicable for videos).

    Returns:
        dict: A dictionary containing the path to the processed file or an error message.
    """
    # Validate the resize factor
    if resize_factor <= 0:
        return {"error": "Resize factor must be a positive integer."}

    # Convert resize factor to a multiplier
    resize_multiplier = resize_factor / 100

    # Set default output path if not provided
    if not output_path:
        output_path = os.path.join("media", "processed")
        os.makedirs(output_path, exist_ok=True)

    # Log processing parameters
    print(f"\nStarting processing with the following parameters:")
    print(f"  File Path: {file_path}")
    print(f"  AI Model: {ai_model}")
    print(f"  Resize Factor: {resize_multiplier * 100}%")
    ...
    try:
        if check_if_file_is_video(file_path):
            ...
            upscale_video(
                ...
                AI_instance=AI(ai_model, "GPU 1", resize_multiplier, 10000),
                AI_instance_list=[AI(ai_model, "Auto", resize_multiplier, 10000)],
                selected_AI_model=ai_model,
                resize_factor=resize_multiplier,
                cpu_number=cpu_number,
                selected_video_extension=".mp4",
                selected_interpolation_factor=0.5,
                selected_AI_multithreading=3,
                selected_keep_frames=keep_frames,
            )
            processed_file_path = find_latest_file(output_path)
        else:
            # Process image
            upscale_image(
                processing_queue=multiprocessing_Queue(),
                ...
                file_number=1,
                selected_output_path=output_path,
                AI_instance=AI(ai_model, "Auto", resize_multiplier, 1000),
                selected_AI_model=ai_model,
                selected_image_extension=".png",
                resize_factor=resize_multiplier,
                selected_interpolation_factor=0.5,
            )
            processed_file_path = os.path.join(output_path, os.path.basename(file_path))
        ...
        return {"error": str(e)}
def process_media_shortform(file_path, ai_model="IRCNN_Lx1", resize_factor=50, output_path=None, cpu_number=4, keep_frames=False):
    """
    Process a video or image file using the specified AI model and parameters.
