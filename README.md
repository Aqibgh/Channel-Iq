...
Video-processing code prefers the bundled Windows executables when available, then falls back to `ffmpeg` and `exiftool` on PATH. The lightweight Docker demo keeps those full media dependencies out of the startup image; install the full backend requirements and system tools for processing deployments. The repository's current remote has Git LFS disabled, so the historical Windows executable objects may remain unavailable until the owner restores LFS storage.
